...
public class ExplosionListener implements Listener {
	...
	public void onBlockDestroy(EntityExplodeEvent event) {
		if ((!event.isCancelled()) && (event.getEntityType() != EntityType.ENDER_DRAGON) &&
	      ...
	      correctExplosion(event, this.entityPowerMap.get(Integer.valueOf(event.getEntity().getEntityId())).floatValue());
	    		...
	    		for (int k = 0; k < 16; k++) {
	    			if ((i == 0) || (i == 15) || (j == 0) || (j == 15) || (k == 0) || (k == 15)) {
	    				...
	    				for (float f2 = 0.3F; f1 > 0.0F; f1 -= f2 * 0.75F) {
		    				...
		    				boolean isInLiquid = world.getBlockAt(l, j1, i1).isLiquid();
		    				
	    					if ((k1 > 0) && (!isInLiquid) && (k1 != 11)) {
	    					...
	    					if ((f1 > 0.0F) && (i1 < 256) && (i1 >= 0)) {
	    						...
	    						if(!isInLiquid){
	    							...
	    							if((block.getType().equals(Material.MOB_SPAWNER)) && (!event.blockList().contains(block))){
	    								event.blockList().add(block);
	    							}
	    						}
	    					}
